...
MEInfo MeTa::getMEInfo() throw(GsmException)
{
  ...
  result._manufacturer =
    stringVectorToString(_at->chatv("+GMI", "+GMI:", false));
  result._model = stringVectorToString(_at->chatv("+GMM", "+GMM", false));
  result._revision =
    stringVectorToString(_at->chatv("+GMR", "+GMR:", false));
  result._serialNumber =
    stringVectorToString(_at->chatv("+GSN", "+GSN:", false),0);
  return result;
}
